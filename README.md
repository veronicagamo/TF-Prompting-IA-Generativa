...
**NutriAgent AI** es una aplicación web interactiva y un agente de inteligencia artificial diseñado para la planificación y optimización de dietas deportivas personalizadas. Utiliza la fórmula metabólica de **Mifflin-St Jeor** para el cálculo preciso de requerimientos energéticos y un **Motor de Escalado Dinámico** calibrado con la base de datos oficial de alimentos de la **Universidad Complutense de Madrid (UCM)** para ajustar de forma exacta las porciones de alimentos a los objetivos nutricionales del usuario.
...
graph TD
    ...
    subgraph Orquestador de IA
        ...
        Agent -->|2. Escalar Recetas Base| ScaleEngine[Motor de Escalado Dinámico]
        ScaleEngine -->|Validar Macronutrientes| UCM_DB[(UCM Database - JSON)]
        Agent -->|3. Redactar Menú| ToolDiet[generar_dieta_tool - tools.py]
        ToolDiet -->|Prompt Optimizado| LLM[Ollama Local qwen2.5]
        LLM -->|Estructura de Platos| Agent
    end
    
    Agent -->|Streaming de Actividades| UI
    Agent -->|Plan Nutricional de Alta Fidelidad| UI
...
1. **Dashboard Web Premium e Interactivo (`app.py`)**:
   - Diseño moderno con un tema oscuro elegante y efectos de **glassmorphism** a través de CSS personalizado.
...
2. **Orquestación de Agentes con Streaming en Tiempo Real (`agent.py`)**:
   - Ejecución secuencial determinista de herramientas que emula la interfaz del clásico `AgentExecutor` de LangChain.
   - **Variabilidad de Menús**: Rotación aleatoria del día base de la semana para que las propuestas de dietas de 1 día nunca sean idénticas.
   - **Motor de Reemplazo Inteligente Culinario**: Clasifica los alimentos preferidos y excluidos del usuario en 8 categorías culinarias específicas (cereales de desayuno, frutas, panes, frutos secos, aceites de aderezo, proteínas de desayuno, proteínas de comida, y carbohidratos salados de comida) para asegurar combinaciones realistas y coherentes (p. ej. las alcachofas no se usarán para desayunar, sino que reemplazarán carbohidratos salados en almuerzo/cena).
...
3. **Motor de Escalado Dinámico Nutricional**:
   - Implementa un algoritmo matemático que toma recetas base (optimizadas matemáticamente para un perfil estándar) y las escala proporcionalmente (basándose en los macronutrientes predominantes de cada ingrediente) hasta alcanzar con alta precisión el objetivo calórico y de macronutrientes diario del usuario.
   - Vinculado directamente con los datos de densidad nutricional oficiales de la **Guía Nutricional UCM**.
...
4. **Herramientas LangChain Especializadas (`tools.py`)**:
   ...
   - `generar_dieta_tool`: Diseña un menú adaptado interactuando con el modelo de lenguaje de Ollama usando temperaturas bajas (0.2) para maximizar la fidelidad y reglas de formato concisas.
...
*   **`agent.py`**: Motor de ejecución del agente. Contiene la lógica de la clase `SequentialNutritionAgentExecutor` y el algoritmo del motor de escalado de porciones.
...
```bash
pip install streamlit langchain langchain-core langchain-ollama pypdf requests
```
...
* Realizar el pull automático de `qwen2.5:3b` (modelo por defecto) y `qwen2.5:1.5b` (modelo alternativo de alto rendimiento en CPU).
